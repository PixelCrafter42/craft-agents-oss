...
/** UI-facing messaging channels registered when a gateway registry is available. */
...
export function registerMessagingHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.messaging.WX_START_CONNECT, async (ctx) => {
    if (!ctx.workspaceId) throw new Error('Missing workspaceId')
    await registry.startWeixinConnect(ctx.workspaceId)
    return { success: true }
  })
